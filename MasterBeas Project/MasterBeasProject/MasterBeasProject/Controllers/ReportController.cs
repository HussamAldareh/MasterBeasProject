            ...
            var document = Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Content().Column(col =>
                    {
                        ...
                        // Photos Section — أضفه بعد Engineer Info وقبل Footer
                        col.Item().Height(10);
                        ...
                        if (report.Images != null && report.Images.Any())
                        {
                            col.Item().Border(1).BorderColor("#DDDDDD").Padding(12).Column(c =>
                            {
                                c.Item().Text("Inspection Photos").Bold().FontColor("#1D9E75");
                                c.Item().Height(8);

                                var rows = report.Images
                                    ...
                                    c.Item().Row(row =>
                                    {
                                        foreach (var item in rowGroup)
                                        {
                                            var physicalPath = Path.Combine(
                                                ...
                                                "wwwroot",
                                                item.img.ImageUrl.TrimStart('/').Replace('/', Path.DirectorySeparatorChar)
                                            );
                                            ...
                                            row.RelativeItem().Padding(4).Column(imgCol =>
                                            {
                                                if (System.IO.File.Exists(physicalPath))
                                                {
                                                    ...
                                                    imgCol.Item().MaxHeight(150).Image(imageBytes);
                                                }
                                                else
                                                {
                                                    imgCol.Item().Text("Image not found").FontColor("#999999").FontSize(9);
                                                }

                                                if (!string.IsNullOrEmpty(item.img.Caption))
                                                    imgCol.Item().Text(item.img.Caption).FontSize(9).FontColor("#666666").AlignCenter();
                                            });
                                        }

                                        // لو الصف فيه صورة وحدة بس، نضيف مكان فاضي
                                        if (rowGroup.Count() == 1)
                                            row.RelativeItem();
                                    });
                                }
